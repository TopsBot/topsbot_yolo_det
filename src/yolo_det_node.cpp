...
topsbot_yolo::YoloDetectorConfig YoloDetNode::load_config()
{
  ...
  publish_detections_ = get_param(*this, "publish_detections", true);
...
void YoloDetNode::initialize()
{
  ...
  RCLCPP_INFO(
    get_logger(),
    "yolo_det ready: model_type=%s model=%s input=%s topic=%s detections=%s publish_det=%s",
    cfg_.model_type.c_str(), cfg_.model_path.c_str(), input_mode_.c_str(),
    image_topic_.c_str(), detections_topic_.c_str(),
    publish_detections_ ? "true" : "false");
  initialized_ = true;
}
...
void YoloDetNode::run_file_mode_once()
{
  ...
  cv::Mat viz_bgr;
  if (!session_.process_with_viz(allocator_, nv12->image, detections, timing, viz_bgr)) {
  ...
  const builtin_interfaces::msg::Time & stamp)
{
  ...
  std::vector<topsbot_yolo::Detection> detections;
  topsbot_yolo::InferenceTiming timing;
  if (!session_.process(allocator_, src_nv12, detections, timing)) {
  ...
  if (enable_profile_) {
    const float total = timing.preprocess_ms + timing.infer_ms + timing.postprocess_ms;
    RCLCPP_INFO(
      get_logger(),
      "frame #%d pre=%.2f infer=%.2f post=%.2f total=%.2f ms dets=%zu",

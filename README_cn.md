...
`topsbot_yolo_det` 在 TOPSBOT 板端运行 YOLO 目标检测：订阅相机 NV12 图像，经 NPU 推理（letterbox + `ta_runtime`），发布结构化检测结果 `/detections`（`tb_det_msgs/TbPerceptionTargets`）。检测框坐标已还原到**原图像素空间**。

在线预览与画框由 **`topsbot_webviz`**（浏览器 Canvas 叠加）负责；本节点专注推理，不再发布画框图像。

典型链路：

```
USB 相机 → /tbmem_img (NV12) → topsbot_yolo_det → /detections
                                              ↘
topsbot_webviz ← /tbmem_img + /detections → 浏览器
```
...
| 4 | `topsbot_webviz`（可选） | Web 预览与检测框叠加 |
...
### 方式二：相机 + YOLO + Web 预览（推荐）

完整 demo，浏览器显示检测框（需安装 `topsbot_webviz`）：

```bash
ros2 launch topsbot_webviz scenario.launch.py scenario:=nv12_yolo
# 浏览器打开 http://<板卡IP>:8000/
```
...
不依赖 ROS 话题，用本地 JPG 验证模型与画框效果，输出带框 JPG：
...
配合 `topsbot_webviz` 场景 `nv12_yolo`：
...
**预期**：`/detections` 以接近相机帧率发布 `TbPerceptionTargets`；`header.stamp` 与对应 `/tbmem_img` 的 `time_stamp` 一致。
...
| 名称 | 消息类型 | 说明 |
| ---- | -------- | ---- |
| `/detections` | `tb_det_msgs/msg/TbPerceptionTargets` | 原图坐标系下的结构化检测（**默认开启**） |

> **v0.2.0 破坏性变更**：已移除 `/yolo_viz`（`sensor_msgs/Image` bgr8 画框图）。请使用 `topsbot_webviz` 预览，或使用 `yolo_det_file_cli` / `input_mode:=file` 做离线 JPG 输出。
...
| `detections_topic` | 检测输出话题 | `/detections` |
| `publish_detections` | 是否发布 `/detections` | `true` |
...
| `file` | 读取本地 JPG，写画框输出后退出（见 `config/yolo_file_demo.yaml`） |
...
- 相机须为 **NV12** 零拷贝输出；请使用 `topsbot_usb_cam` 的 `mjpeg2nv12_zerocopy` preset
- 确认 `publish_detections:=true`（v0.2.0 起为默认值）
...
- 增大 `frame_skip` 跳帧
- 查看日志中 `enable_profile:=true` 打印的 pre/infer/post 耗时，定位瓶颈

**6. 如何只做一次离线验证、不接相机？**

使用 `yolo_det_file_cli`（见上文「方式四」），或用 `input_mode:=file` 加载 `config/yolo_file_demo.yaml`。
...
- 已移除 `/yolo_viz`；请订阅 `/detections` 或使用 `topsbot_webviz`
- `publish_detections` 默认改为 `true`
- 已删除 `viz_topic` 参数
